...
{
    [MenuItem("GameObject/Lua Tool(CA)/生成UI解析代码(Shift+U) #U", false, 1)]
   ...
   public string buttonRegisterArray = @"
    ...
    private void AttackLuaScript(bool force =false)
    {
        ...
        if(behaviour == null)
        {
            EditorUtility.DisplayDialog("提示", "没有找到LuaBehaviour脚本，请手动添加!", "确定");
            return;
        }
    ...
    void OnGUI()
    {
        ...
        GUILayout.BeginHorizontal();
        GUILayout.Label("脚本路径:"+ MakeModulePath(), GUILayout.Width(330));
    ...
    public static void ShowBehaviorView()
    {
        ...
        LuaUiGenerator w = (LuaUiGenerator)GetWindowWithRect(typeof(LuaUiGenerator), r, true, "生成Lua View代码");
        w.LoadConfigure();
