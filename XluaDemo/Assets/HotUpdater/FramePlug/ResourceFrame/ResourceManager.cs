...
public class ResourceManager : Singleton<ResourceManager>
{
    ...
    //资源加载器
    public AssetsLoader mAssetLoader = new AssetsLoader();
    //Mono脚本
    ...
    //所有图集集合
    protected Dictionary<string, Object[]> m_AllAtlasDic = new Dictionary<string, Object[]>();
    //最长连续卡着加载资源的时间，单位微妙
    ...
    public void Init(MonoBehaviour mono,bool LoadFromBundle)
    {
...
#if UNITY_EDITOR
        m_LoadFormAssetBundle = LoadFromBundle;
#endif
        m_Startmono = mono;
        m_Startmono.StartCoroutine(AsyncLoadCor());
        Debug.LogError("是否AssetBundle加载："+ m_LoadFormAssetBundle);
    }
    ...
    /// 加载图集图片
    /// </summary>
    public Sprite LoadAtlasSprite(string path, string name)
    {
        if (string.IsNullOrEmpty(path))
        {
            return null;
        ...
        if (m_AllAtlasDic.TryGetValue(path ,out spriteObjs)&&spriteObjs!=null)
        {
            return LoadSpriteFormAltas(spriteObjs, name);
        }
        uint crc = Crc32.GetCrc32(path);
        ResouceItem item = GetCacheResouceItem(crc);
        if (item != null)
        {
            Object[] objects = item.m_AssetBundle.LoadAllAssets();
        ...
        if (!m_LoadFormAssetBundle)
        {
            spriteObjs = UnityEditor.AssetDatabase.LoadAllAssetsAtPath(path);
            Object[] objects = null;
            if (!m_AllAtlasDic.TryGetValue(path,out objects)|| objects==null)
            {
                m_AllAtlasDic.Add(path, spriteObjs);
            }
            return LoadSpriteFormAltas(spriteObjs, name);
        }
#endif
    ...
    /// <returns></returns>
    public Sprite LoadSpriteFormAltas(Object[] objects, string name)
    {
