...
public class AssetBundleManager : Singleton<AssetBundleManager>
{
    ...
    public bool LoadAssetBundleConfig()
    {
        ...
        byte[] bytes = AES.AESFileByteDecrypt(configPath, "YYCM");
    ...
    public ResouceItem LoadResouceAssetBundle(uint crc)
    {
        ...
        {
            Debug.LogError(string.Format("LoadResourceAssetBundle error: can not find crc {0} in AssetBundleConfig", crc.ToString())+new System.Diagnostics.StackTrace().ToString());
            return item;
        }
    ...
    private AssetBundle LoadAssetBundle(string name)
    {
        ...
        if (!m_AssetBundleItemDic.TryGetValue(crc, out item))
        {
            ...
            byte[] bytes = AES.AESFileByteDecrypt(fullPath, "YYCM");
