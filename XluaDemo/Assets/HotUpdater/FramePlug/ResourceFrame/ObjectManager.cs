...
public class ObjectManager : Singleton<ObjectManager>
{
    ...
    public void Init(Transform recycleTrs,Transform sceneTrs,Transform uiRot)
    {
    ...
    {
        return m_ResouceObjDic[obj.GetInstanceID()] == null ? false : true;
    }
    ...
    {
        path= path.EndsWith(".prefab") == false ? path += ".prefab" : path;
    ...
    /// <param name="ifParentIsNullIfSetScene">如果父物体为空 是否设置到场景里</param>
    /// <returns></returns>
    public GameObject InstantiateObject(string path, Transform parent,bool LocalPosIsZroe=true,bool localScaleIsOne=true, bool bClear = true)
    {
        path = path.EndsWith(".prefab") == false ? path += ".prefab" : path;
        uint crc = Crc32.GetCrc32(path);
        ResouceObj resouceObj = GetObjectFromPool(crc);
        ...
        //如果父物体为空 就默认设置在场景中  以免在回收节点下不显示
        if (parent==null)
            resouceObj.m_CloneObj.transform.SetParent(null);
        //如果不为空就设置位置和父物体
        if (parent != null)
        {
            resouceObj.m_CloneObj.transform.SetParent(parent, false);
            if (LocalPosIsZroe)
                resouceObj.m_CloneObj.transform.localPosition = Vector3.zero;
            if (localScaleIsOne)
                resouceObj.m_CloneObj.transform.localScale = Vector3.one;
        }
    ...
    /// <param name="path"></param>
    /// <returns></returns>
    public GameObject InstantiateView(string path)
    {
      return  InstantiateObject(path,UIRoot);
    }
    /// <summary>
    /// 异步对象加载
    /// </summary>
    ...
    /// <param name="destoryCache">是否从对象池中销毁 不销毁则回收到对象池</param>
    /// <param name="recycleParent"></param>
    public void ReleaseObject(GameObject obj, bool destoryCache = false, int maxCacheCount = -1, bool recycleParent = true)
    {
        if (obj == null)
            return;
        ...
        else//回收到对象池
        {
            ...
            {
                //Debuger.Log("即将回收物体："+resObj.m_CloneObj.GetInstanceID());
                if (recycleParent)
                {
                    //Debuger.Log("回收成功 设置父物体:" + resObj.m_CloneObj.GetInstanceID());
                    resObj.m_CloneObj.transform.SetParent(RecyclePoolTrs);
                }
                else
                {
                    //Debuger.Log("没有回收到场景节点 直接隐藏");
