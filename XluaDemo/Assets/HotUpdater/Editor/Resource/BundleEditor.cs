...
{
    private static string ABDownPath;
    ...
    private static string ABCONFIGPATH = "Assets/HotUpdater/Editor/Resource/ABConfig.asset";
    ...
    {
        AES.AESFileEncrypt(Application.dataPath + "/GameData/Data/Xml/TestData.xml", "YYCM");
    }

    //[MenuItem("测试/测试解密")]
    public static void TestDec()
    {
        AES.AESFileDecrypt(Application.dataPath + "/GameData/Data/Xml/TestData.xml", "YYCM");
    }
    ...
    public static void EncryptAB()
    {
        ...
        {
            EditorUtility.DisplayProgressBar("加密AssetBundle", "name:" + files[i].Name, i * 1.0f / files.Length);
            if (!files[i].Name.EndsWith("meta") && !files[i].Name.EndsWith(".manifest"))
            {
                AES.AESFileEncrypt(files[i].FullName, "YYCM");
            }
        }
        Debug.Log("加密完成！");
    }
    ...
    public static void DecrptyAB()
    {
        ...
        for (int i = 0; i < files.Length; i++)
        {
            if (!files[i].Name.EndsWith("meta") && !files[i].Name.EndsWith(".manifest"))
            {
                AES.AESFileDecrypt(files[i].FullName, "YYCM");
            }
        }
        Debug.Log("加密完成！");
    }

    [MenuItem("Tools/AssetBundle/打AB包")]
    ...
    {
        ABDownPath = GameDefine.Instanse.AssetBundleDownURL+ "/AssetBundle/";
        m_ConfigFil.Clear();
        m_AllFileAB.Clear();
        m_AllFileDir.Clear();
        ...
        {
            index++;
            EditorUtility.DisplayProgressBar("查找AB文件", "name:" + fileDir.ABName, index * 1.0f / abConfig.m_AllFileDirAB.Count);
        ...
        for (int i = 0; i < allStr.Length; i++)
        {
            ...
            if (!ContainAllFileAB(path))
            {
                ...
                {
                    EditorUtility.DisplayProgressBar("查找依赖项", "Depend:" + allDepend[j], j * 1.0f / allDepend.Length);
        ...
        {
            index++;
            EditorUtility.DisplayProgressBar("设置AssetBundle包名", "name:" + name, index * 1.0f / m_AllFileDir.Count);
        ...
        EditorUtility.DisplayProgressBar("打包完成", " 请稍等...",1);
    ...
    static void WriteABMD5()
    {
        ...
        for (int i = 0; i < files.Length; i++)
        {
            EditorUtility.DisplayProgressBar("写入ABND5版本信息", "名字：" + files[i].Name, i * 1.0f / files.Length);
            if (!files[i].Name.EndsWith(".meta") && !files[i].Name.EndsWith(".manifest"))
            {
    ...
    static void ReadMd5Com(string abmd5Path, string hotCount)
    {
        ...
        for (int i = 0; i < files.Length; i++)
        {
            EditorUtility.DisplayProgressBar("写入MD5", "Name：" + files[i].Name, i * 1.0f / files.Length);
            if (!files[i].Name.EndsWith(".meta") && !files[i].Name.EndsWith(".manifest"))
            {
    ...
    static void CopyABAndGeneratXml(List<string> changeList, string hotCount)
    {
        ...
        foreach (string str in changeList)
        {
            ...
            {
                EditorUtility.DisplayProgressBar("拷贝热更文件", "Name：" + str, index * 1.0f / changeList.Count);
        ...
        serverInfo.GameVersion  = new VersionInfo [1];
        serverInfo.GameVersion[0] = versionInfo;
        versionInfo.Version = PlayerSettings.bundleVersion;
        Pathces pathces = new Pathces();
        pathces.Des = "修复部分已知bug";
        pathces.Version = 1;
        pathces.Files = new List<Patch>();
        for (int i = 0; i < files.Length; i++)
        {
            EditorUtility.DisplayProgressBar("写入补丁信息", "Name：" + files[i].Name, i * 1.0f / files.Length);
            ...
            patch.Url = ABDownPath + PlayerSettings.bundleVersion + "/" + hotCount + "/" + files[i].Name;
            pathces.Files.Add(patch);
        }
        versionInfo.Pathces = new Pathces[1];
        versionInfo.Pathces[0] = pathces;
        BinarySerializeOpt.Xmlserialize(m_HotPath + "/ServerInfo.xml", serverInfo);
    }
    ...
    static void BunildAssetBundle()
    {
        ...
        {
            EditorUtility.DisplayProgressBar("筛选需要打包的AssetBundle", "name:" + allBundles[i], i * 1.0f / allBundles.Length);
        ...
        EditorUtility.DisplayProgressBar("开始打包AssetBundle", "准备资源中请稍等..." ,1);
    ...
    static void DeleteMainfest()
    {
        ...
        for (int i = 0; i < files.Length; i++)
        {
            EditorUtility.DisplayProgressBar("清空.nabufest文件", "name:" + files[i].Name, i * 1.0f / files.Length);
    ...
    static void WriteData(Dictionary<string ,string> resPathDic)
    {
        ...
        foreach (string path in resPathDic.Keys)
        {
            ...
            if (!ValidPath(path))
                continue;
            EditorUtility.DisplayProgressBar("写入AssetBundle配置", "ABName:" + resPathDic[path], index * 1.0f / resPathDic.Count);
            ...
            for (int i = 0; i < resDependce.Length; i++)
            {
                ...
                EditorUtility.DisplayProgressBar("写入依赖项", "Dependce:" + tempPath, i * 1.0f / resDependce.Length);
    ...
    static void DeleteAB()
    {
        ...
        for (int i = 0; i < files.Length; i++)
        {
            EditorUtility.DisplayProgressBar("删除冗余AssetBundle", "files[i].Name,:" + files[i].Name, i * 1.0f / files.Length);
    ...
    public static void DeleteAllFile(string fullPath)
    {
        if (Directory.Exists(fullPath))
        {
            ...
            for (int i = 0; i < files.Length; i++)
            {
                EditorUtility.DisplayProgressBar("删除所有热更文件", "Name：" + files[i].Name, i * 1.0f / files.Length);
